...
def seed_database(request):
    """Seed database with Egyptian locations and lockers"""
    ...
    try:
        with DatabaseConnection.get_connection() as conn:
            ...
            cursor.execute("SELECT COUNT(*) as cnt FROM lockers_lockerlocation")
            if cursor.fetchone()['cnt'] > 0:
                cursor.close()
                return Response({'message': 'Database already seeded', 'status': 'skipped'})
            
            # Create pricing tiers (include created_at)
            ...
            cursor.execute("SELECT COUNT(*) as cnt FROM lockers_lockerunit")
            locker_count = cursor.fetchone()['cnt']
            cursor.close()
            ...
            return Response({
                ...
                'lockers_created': locker_count,
                'location_names': created_locations
            })
